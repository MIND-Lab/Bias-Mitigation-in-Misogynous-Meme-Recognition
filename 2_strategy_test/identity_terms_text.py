...
if not os.path.exists('./IdentityTerms_myprepro/'):
    os.makedirs('./IdentityTerms_myprepro/')
...
data = load_data.load_training_data_Spacy()
data['clear text'] = ''
...
for index, row in tqdm(data.iterrows()):
    #print(row[2])
    data.loc[index, 'clear text'] = str(data_pre_processing(row[2])).replace("'", '').replace(",", '').replace("[",
                                                                                                            '').replace(
        "]", '').replace("\"", '')
...
word.to_csv('./IdentityTerms_myprepro/lemma_presence_stanza.csv')
...
scores_df.to_csv('./IdentityTerms_myprepro/scores_Lemma_Stanza.csv', index=False)
...
# first/last 10 terms
identity_misogynous = scores_df[0:10].word.tolist()
identity_non_misogynous = scores_df[scores_df.shape[0] - 10:scores_df.shape[0]].word.tolist()
identity_non_misogynous.reverse()

identity_terms = [identity_misogynous, identity_non_misogynous]

with open('./data/IdentityTerms_myprepro.txt', 'w') as f:
