...
def compute_subgroup_auc(df, subgroup, label, model_name):
    ...
    #print("compute_subgroup_auc ",subgroup_examples)
    auc = compute_auc(subgroup_examples[label], subgroup_examples[model_name])
    #print("compute_subgroup_auc ", auc)
    return auc
...
def compute_negative_cross_auc(df, subgroup, label, model_name):
    ...
    #print("compute_negative_cross_auc ",examples)
...
#    #print(df[label_col])
#    #print(df)
#    y_true = list(df[label_col])
#    probs = list(df[score_col])
#    #print(size)
#    y_pred = []
#    for item in probs:
#        if item >= threshold:
...
def compute_confusion_rates(df, score_col, label_col, threshold, size):
    
    confusion = confusion_matrix_counts(df[:size], score_col, label_col, threshold)
    ...
    auc = compute_auc(df[label_col][:size], df[score_col][:size])
...
def model_family_auc_10Fold_Syn(dataset, model_names, size):
    """
    Compute Auc score for every model in the model family.
    ...
    i = 0
    for model_name in model_names:
        aucs.append(compute_auc(dataset['label_'+model_name][:size[i]], dataset[model_name][:size[i]]))
        i+=1
    return {
        'aucs': aucs,
        'mean': np.mean(aucs),
